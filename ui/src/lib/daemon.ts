...
export function cliToolProtocolFamilies(
  ...
  const next: string[] = [];
  for (const raw of [
    ...(tool?.protocol_families ?? []),
    tool?.protocol_family ?? "",
  ]) {
...
export function cliToolSupportsProvider(
  tool:
    | Pick<CLITool, "protocol_family" | "protocol_families">
    | null
    | undefined,
...
export async function fetchCLIToolSettings(
  daemonUrl: string,
): Promise<CLIToolSettings> {
...
export async function putCLIToolSettings(
  daemonUrl: string,
  req: PutCLIToolSettingsRequest,
): Promise<CLIToolSettings> {
  const res = await fetch(`${daemonUrl}/api/v1/settings/cli-tools`, {
    method: "PUT",
...
export async function startIFLOWBrowserAuth(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/settings/cli-tools/iflow/browser-auth`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(req ?? {}),
...
export async function fetchIFLOWBrowserAuth(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/settings/cli-tools/iflow/browser-auth/${sessionId}`,
  );
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new Error(text || `HTTP ${res.status} ${res.statusText}`.trim());
...
export async function submitIFLOWBrowserAuthCode(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/settings/cli-tools/iflow/browser-auth/${sessionId}/code`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(req),
...
export async function cancelIFLOWBrowserAuth(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/settings/cli-tools/iflow/browser-auth/${sessionId}/cancel`,
    {
      method: "POST",
    },
  );
...
export async function fetchSkillSettings(
  daemonUrl: string,
...
export async function postTranslateText(
  daemonUrl: string,
  text: string,
  ...
  if (!res.ok) {
    ...
    throw new Error(
      data?.error ?? `HTTP ${res.status} ${res.statusText}`.trim(),
    );
  }
...
export type APISourceSettings = {
  sources: APISource[];
};

export type PutAPISourceSettingsRequest = {
  sources: Array<{
    id: string;
    label: string;
    base_url?: string;
...
export type RuntimeModelRuntime = {
  id: string;
  label: string;
  kind: "sdk" | "cli";
  provider?: string;
...
export type RuntimeModelSettings = {
  runtimes: RuntimeModelRuntime[];
};

export type PutRuntimeModelSettingsRequest = {
  runtimes: Array<{
    id: string;
    default_model_id?: string;
    models: Array<{
...
export async function fetchAPISourceSettings(
  daemonUrl: string,
): Promise<APISourceSettings> {
  const res = await fetch(`${daemonUrl}/api/v1/settings/api-sources`);
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new Error(text || `HTTP ${res.status} ${res.statusText}`.trim());
  }
  return (await res.json()) as APISourceSettings;
}

export async function putAPISourceSettings(
  daemonUrl: string,
  req: PutAPISourceSettingsRequest,
): Promise<APISourceSettings> {
  const res = await fetch(`${daemonUrl}/api/v1/settings/api-sources`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
...
export async function fetchRuntimeModelSettings(
  daemonUrl: string,
): Promise<RuntimeModelSettings> {
  const res = await fetch(`${daemonUrl}/api/v1/settings/runtime-models`);
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new Error(text || `HTTP ${res.status} ${res.statusText}`.trim());
  }
  return (await res.json()) as RuntimeModelSettings;
}

export async function putRuntimeModelSettings(
  daemonUrl: string,
  req: PutRuntimeModelSettingsRequest,
): Promise<RuntimeModelSettings> {
  const res = await fetch(`${daemonUrl}/api/v1/settings/runtime-models`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
...
export async function fetchBasicSettings(
  daemonUrl: string,
...
export async function fetchExpertBuilderSessions(
  ...
  if (params?.targetExpertId)
    url.searchParams.set("target_expert_id", params.targetExpertId);
...
export async function fetchExpertBuilderSession(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/settings/experts/sessions/${sessionId}`,
  );
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new Error(text || `HTTP ${res.status} ${res.statusText}`.trim());
...
export async function postExpertBuilderMessage(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/settings/experts/sessions/${sessionId}/messages`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(req),
...
export async function postExpertBuilderPublish(
  ...
  session: ExpertBuilderSession;
  published_expert: ExpertSettingsItem;
  messages: ExpertBuilderMessageItem[];
  snapshots: ExpertBuilderSnapshot[];
}> {
  const res = await fetch(
    `${daemonUrl}/api/v1/settings/experts/sessions/${sessionId}/publish`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function postChatTurn(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/chat/sessions/${sessionId}/turns`,
    init,
  );
  if (!res.ok) {
    const text = await res.text().catch(() => "");
...
export async function postChatCompact(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/chat/sessions/${sessionId}/compact`,
    {
      method: "POST",
    },
  );
...
export async function postChatFork(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/chat/sessions/${sessionId}/fork`,
    {
      method: "POST",
...
export async function fetchOrchestrationDetail(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/orchestrations/${orchestrationId}`,
  );
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new Error(text || `HTTP ${res.status} ${res.statusText}`.trim());
...
export async function cancelOrchestration(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/orchestrations/${orchestrationId}/cancel`,
    {
      method: "POST",
    },
  );
...
export async function continueOrchestration(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/orchestrations/${orchestrationId}/continue`,
    {
      method: "POST",
    },
  );
...
export async function retryAgentRun(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/agent-runs/${agentRunId}/retry`,
    {
      method: "POST",
    },
  );
...
export async function cancelExecution(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/executions/${executionId}/cancel`,
    {
      method: "POST",
    },
  );
...
export async function approveWorkflow(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/workflows/${workflowId}/approve`,
    {
      method: "POST",
    },
  );
...
export async function cancelWorkflow(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/workflows/${workflowId}/cancel`,
    {
      method: "POST",
    },
  );
    ...
    latest_ref:
      typeof record.latest_resolved_ref === "string"
        ...
        : typeof record.latest_ref === "string"
          ...
          : undefined,
    latest_commit_sha:
      typeof record.latest_commit_sha === "string"
        ? record.latest_commit_sha
        : undefined,
    snapshot_count:
      typeof record.snapshot_count === "number"
...
function normalizeRepoLibraryAnalysisRun(
  ...
  return {
    ...
    runtime_kind:
      typeof record.runtime_kind === "string" ? record.runtime_kind : undefined,
    chat_session_id:
      typeof record.chat_session_id === "string"
        ? record.chat_session_id
        : typeof record.chatSessionId === "string"
          ? record.chatSessionId
          : undefined,
    chat_user_message_id:
      typeof record.chat_user_message_id === "string"
        ? record.chat_user_message_id
        : typeof record.chatUserMessageId === "string"
          ? record.chatUserMessageId
          : undefined,
    chat_assistant_message_id:
      typeof record.chat_assistant_message_id === "string"
        ? record.chat_assistant_message_id
        : typeof record.chatAssistantMessageId === "string"
...
function normalizeRepoLibraryCardEvidence(
  body: unknown,
...
function normalizeRepoLibraryRepositorySummary(
  body: unknown,
...
function normalizeRepoLibraryRepositoryDetail(
  body: unknown,
...
function normalizeRepoLibrarySearchResult(
  body: unknown,
  ...
  return {
    result_id:
      typeof record.result_id === "string"
        ...
        : typeof record.chunk_id === "string"
          ...
          : undefined,
    repository_id: String(
      (repositoryPayload as Record<string, unknown> | null)?.repo_key ??
        (repositoryPayload as Record<string, unknown> | null)?.repository_id ??
        record.repository_id ??
        "",
    ),
    snapshot_id:
    ...
    title:
      typeof record.title === "string"
        ? record.title
        : typeof (record.chunk as Record<string, unknown> | undefined)
              ?.section_title === "string"
          ? String((record.chunk as Record<string, unknown>).section_title)
          : undefined,
    summary:
      typeof record.summary === "string"
        ? record.summary
        : typeof record.text_excerpt === "string"
          ? record.text_excerpt
          : undefined,
    rationale:
    ...
    evidence_preview: Array.isArray(record.evidence_preview)
      ...
      : Array.isArray(record.evidence_refs)
        ? (record.evidence_refs as unknown[]).map((item, index) =>
            normalizeRepoLibraryCardEvidence({
              evidence_id: `ref-${index}`,
              source_path: String(item),
            }),
          )
        : undefined,
  };
...
export async function syncRepoLibraryAnalysisChat(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/repo-library/analyses/${encodeURIComponent(analysisId)}/sync-chat`,
    {
      method: "POST",
    },
  );
...
export async function fetchRepoLibraryRepository(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/repo-library/repositories/${repositoryId}`,
  );
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new Error(text || `HTTP ${res.status} ${res.statusText}`.trim());
...
export async function fetchRepoLibraryCards(
  ...
  return readRepoLibraryList<unknown>(await res.json(), ["items", "cards"]).map(
    (item) => normalizeRepoLibraryCard(item),
  );
}
...
export async function fetchRepoLibraryCardEvidence(
  ...
  const res = await fetch(
    `${daemonUrl}/api/v1/repo-library/cards/${cardId}/evidence`,
  );
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new Error(text || `HTTP ${res.status} ${res.statusText}`.trim());
...
export async function searchRepoLibrary(
  ...
  if (Array.isArray(record.results)) {
    return {
      query_id:
        typeof record.query_id === "string" ? record.query_id : undefined,
      results: (record.results as unknown[]).map((item) =>
        normalizeRepoLibrarySearchResult(item),
      ),
    };
  }
  return {
    query_id: typeof record.query_id === "string" ? record.query_id : undefined,
    results: readRepoLibraryList<unknown>(body, ["items"]).map((item) =>
      normalizeRepoLibrarySearchResult(item),
    ),
  };
}
