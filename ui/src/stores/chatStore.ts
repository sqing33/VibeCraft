...
type LiveDeltaBatch = {
  ...
  thinkingTranslationStateBySession?: Record<string, ThinkingTranslationState>;
...
function appendTextDeltas(
  ...
  for (const [sessionId, delta] of Object.entries(deltas)) {
    if (!delta) continue;
    if (!next) next = { ...current };
    next[sessionId] = `${next[sessionId] ?? ""}${delta}`;
  }
  return next ?? current;
}

function applyLiveDeltaBatchShape<
  T extends {
    streamingBySession: Record<string, string>;
    thinkingBySession: Record<string, string>;
    ...
    thinkingTranslationStateBySession: Record<
      string,
      ThinkingTranslationState | undefined
    >;
    activeTurnFeedBySession: Record<string, ChatTurnFeed | undefined>;
  },
>(state: T, batch: LiveDeltaBatch): T {
  let nextState = state;

  const nextStreaming = appendTextDeltas(
    nextState.streamingBySession,
    batch.streamingBySession,
  );
  if (nextStreaming !== nextState.streamingBySession) {
    nextState = { ...nextState, streamingBySession: nextStreaming };
  }

  const nextThinking = appendTextDeltas(
    nextState.thinkingBySession,
    batch.thinkingBySession,
  );
  if (nextThinking !== nextState.thinkingBySession) {
    nextState = { ...nextState, thinkingBySession: nextThinking };
  }

  if (Array.isArray(batch.turnEvents) && batch.turnEvents.length > 0) {
    let nextActiveFeed = nextState.activeTurnFeedBySession;
    for (const event of batch.turnEvents) {
      const prevFeed = nextActiveFeed[event.session_id];
      const updatedFeed = applyTurnFeedEvent(prevFeed, event);
      if (updatedFeed === prevFeed) continue;
      if (nextActiveFeed === nextState.activeTurnFeedBySession) {
        nextActiveFeed = { ...nextActiveFeed };
      }
      nextActiveFeed[event.session_id] = updatedFeed;
    }
    if (nextActiveFeed !== nextState.activeTurnFeedBySession) {
      nextState = { ...nextState, activeTurnFeedBySession: nextActiveFeed };
    }
  }

  if (
    Array.isArray(batch.translatedThinkingDeltas) &&
    batch.translatedThinkingDeltas.length > 0
  ) {
    let nextTranslatedThinking = nextState.translatedThinkingBySession;
    let nextActiveFeed = nextState.activeTurnFeedBySession;
    for (const item of batch.translatedThinkingDeltas) {
      if (!item.delta) continue;
      if (nextTranslatedThinking === nextState.translatedThinkingBySession) {
        nextTranslatedThinking = { ...nextTranslatedThinking };
      }
      nextTranslatedThinking[item.sessionId] =
        `${nextTranslatedThinking[item.sessionId] ?? ""}${item.delta}`;

      const prevFeed = nextActiveFeed[item.sessionId];
      const updatedFeed = applyThinkingTranslationDelta(
        prevFeed,
        item.delta,
        item.entryId,
      );
      ...
      nextActiveFeed[item.sessionId] = updatedFeed;
    }
    if (nextTranslatedThinking !== nextState.translatedThinkingBySession) {
      nextState = {
        ...nextState,
  ...
  if (batch.thinkingTranslationStateBySession) {
    let nextTranslationState = nextState.thinkingTranslationStateBySession;
    for (const [sessionId, translationState] of Object.entries(
      batch.thinkingTranslationStateBySession,
    )) {
      const prevState = nextTranslationState[sessionId];
      if (
        prevState?.applied === translationState.applied &&
        prevState?.failed === translationState.failed
      )
        continue;
      if (
        nextTranslationState === nextState.thinkingTranslationStateBySession
      ) {
        nextTranslationState = { ...nextTranslationState };
      }
      nextTranslationState[sessionId] = translationState;
    }
    if (nextTranslationState !== nextState.thinkingTranslationStateBySession) {
      nextState = {
        ...nextState,
...
function clearLiveTurnStateShape<
  T extends {
    streamingBySession: Record<string, string>;
    thinkingBySession: Record<string, string>;
    translatedThinkingBySession: Record<string, string>;
...
function shouldClearActiveTurnFeed(
  feed: ChatTurnFeed | undefined,
  messages: ChatMessage[],
): boolean {
  if (!feed || messages.length === 0) return false;
  const userMessage = messages.find(
    (message) =>
      message.message_id === feed.user_message_id && message.role === "user",
  );
  if (!userMessage) return false;
  return messages.some(
    (message) =>
      message.role === "assistant" && message.turn === userMessage.turn,
  );
}
...
function hydrateTurnsIntoState<
  T extends {
    ...
    activeTurnFeedBySession: Record<string, ChatTurnFeed | undefined>;
    completedTurnFeedByAssistantMessageId: Record<
      string,
...
>(state: T, sessionId: string, turns: ChatTurnTimeline[]): T {
  ...
  const sessionAssistantIDs = new Set(
    sessionMessages
  ...
  for (const turn of turns) {
    ...
    if (
      typeof turn.assistant_message_id === "string" &&
  ...
  const nextCompleted = Object.fromEntries(
    Object.entries(state.completedTurnFeedByAssistantMessageId).filter(
      ([, feed]) => feed?.session_id !== sessionId,
    ),
...
export type ChatStore = {
  ...
  applyLiveDeltaBatch: (batch: LiveDeltaBatch) => void;
  ...
  setThinkingTranslationState: (
    sessionId: string,
    state: ThinkingTranslationState,
  ) => void;
...
export const useChatStore = create<ChatStore>()(
  persist(
    (set, get) => ({
      ...
      appendTranslatedThinkingDelta: (sessionId, delta, entryId) =>
        set((state) => ({
          translatedThinkingBySession: {
            ...state.translatedThinkingBySession,
            [sessionId]:
              (state.translatedThinkingBySession[sessionId] ?? "") + delta,
          },
          activeTurnFeedBySession: {
            ...state.activeTurnFeedBySession,
            [sessionId]: applyThinkingTranslationDelta(
              state.activeTurnFeedBySession[sessionId],
              delta,
              entryId,
            ),
      ...
      applyLiveDeltaBatch: (batch) =>
        set((state) => applyLiveDeltaBatchShape(state, batch)),
      ...
      applyTurnEvent: (event) =>
        set((state) => ({
          activeTurnFeedBySession: {
            ...state.activeTurnFeedBySession,
            [event.session_id]: applyTurnFeedEvent(
              state.activeTurnFeedBySession[event.session_id],
              event,
            ),
          },
        })),
      completeTurnFeed: (sessionId, assistantMessageId, opts) =>
        set((state) => {
          const activeFeed = finalizeTurnFeed(
            state.activeTurnFeedBySession[sessionId],
            assistantMessageId,
            opts,
          );
      ...
      refreshSessions: async (daemonUrl) => {
        ...
        try {
          ...
          const fallbackSessionId =
            activeSessions[0]?.session_id ?? sessions[0]?.session_id ?? null;
          set((state) => {
      ...
      sendTurn: async (
        ...
        const now = Date.now();
        const lastTurn =
          (get().messagesBySession[sessionId] ?? []).at(-1)?.turn ?? 0;
