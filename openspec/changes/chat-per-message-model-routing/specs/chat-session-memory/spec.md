...
### Requirement: Chat turns MAY specify expert_id
The turn API MUST accept an optional `expert_id` field for selecting the expert/provider/model for that turn.

#### Scenario: Turn request includes expert_id
- **WHEN** client posts `POST /api/v1/chat/sessions/:id/turns` with `input` and `expert_id`
- **THEN** the turn executes using the provided expert

### Requirement: Chat messages MUST include model identity metadata
For messages created after this change, the system MUST store and return `expert_id`, `provider`, and `model` for each chat message.

#### Scenario: Messages return expert/provider/model
- **WHEN** a successful turn completes
- **THEN** `GET /api/v1/chat/sessions/:id/messages` returns the user and assistant messages including `expert_id`, `provider`, and `model`
...
The `POST /api/v1/chat/sessions/:id/turns` request MUST accept:
- `input` (required)
- `expert_id` (optional) for per-turn expert selection

The `chat.turn.started` event payload MUST include `expert_id`, `provider`, and `model` used for the turn.
...
#### Scenario: Successful streaming turn with expert override
- **WHEN** client posts `POST /api/v1/chat/sessions/:id/turns` with `input` and `expert_id`
- **THEN** a `chat.turn.started` event is emitted with the requested `expert_id`
- **AND** subsequent streaming events correspond to that turn execution

### Requirement: Provider anchors SHALL be reused with safe fallback
For OpenAI sessions, the system MUST persist and reuse `previous_response_id` when available. For Anthropic sessions, the system MUST persist and reuse `container` when available.

The system MUST NOT reuse anchors across different provider+model combinations. When the current turn uses a different provider+model than the session's current defaults, the system MUST bypass anchor reuse and use reconstructed local context (summary + recent messages).

If anchor usage fails or is unavailable, the system MUST fallback to reconstructed local context and continue.
...
#### Scenario: Anchor bypass when switching model
- **WHEN** a turn uses a different provider+model than the session's current defaults
- **THEN** the turn uses reconstructed local context from local summary + recent messages
- **AND** the provider call does not include the previous anchor
...
Compaction MUST use an LLM-first summary generation policy. If LLM summarization fails, the system MUST fall back to a local deterministic summary and continue.
...
Manual compaction MUST use the same LLM-first summary generation policy and the same fallback behavior as automatic compaction.
