...
Compatibility for a warm runtime MUST include the current effective MCP gateway access policy; when the session MCP selection has changed in a way that makes the warm runtime incompatible, the daemon MUST discard that warm runtime before the next turn starts.
...
#### Scenario: Codex warm runtime is discarded after MCP selection change
- **WHEN** a Codex-backed chat session has a warm app-server runtime
- **AND** the session's effective MCP gateway allowlist changes before the next turn
- **THEN** the daemon discards the incompatible warm runtime
- **AND** the next turn starts with a fresh runtime or cold thread resume path using the updated MCP policy
