...
`vibe-tree` 目前只持久化 MCP 配置，并在少数运行时按 turn 注入配置；它没有常驻的 MCP 运行面，因此对话启动时容易找不到可用 MCP，多个 CLI 的接入方式也不一致。现在需要把 MCP 从“设置项”升级为“daemon 托管能力”，让 CLI 始终只连接一个稳定入口，并支持按需唤醒、热更新选择和统一观测。
