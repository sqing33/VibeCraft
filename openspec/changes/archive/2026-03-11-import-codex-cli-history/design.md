...
Codex CLI 会把历史线程写在 `~/.codex/state_*.sqlite` 的 `threads` 表里，具体 turn 过程则保存在 `threads.rollout_path` 指向的 JSONL 文件中。`vibecraft` 已有完整的 chat transcript 与 turn timeline 数据模型，前端也已经能渲染 `tool`、`thinking`、`progress`、`answer` 等条目，因此这次变更的关键不是新建一套历史展示模型，而是把 Codex 历史稳定映射进现有模型。
...
**Non-Goals:**
- 不导入 `~/.local/share/vibecraft/managed-clis/codex` 的 managed runtime 历史。
